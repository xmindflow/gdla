...
def build_dataset(cfg) -> Dict[str, Callable[[], Dataset]]:
    ...
    return {
        'train': partial(ACDC, data_dir=cfg.DATA.TRAIN_DIR, transform=train_transform),
        'val': partial(ACDC, data_dir=cfg.DATA.VAL_DIR, transform=val_transform),
        'test': partial(ACDC, data_dir=cfg.DATA.TEST_DIR, transform=test_transform)
    }
