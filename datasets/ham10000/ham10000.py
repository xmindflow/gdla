...
class HAM10000(Dataset):
    ...
    def _load(self, data_dir: str) -> Tuple[List[np.ndarray], List[np.ndarray]]:
        ...
        for sub_id in sub_ids:
            img_path = os.path.join(data_dir, sub_id, f"ISIC_{sub_id}.jpg")
            msk_path = os.path.join(data_dir, sub_id, f"ISIC_{sub_id}_segmentation.png")
...
@DATASET.register(name="ham10000")
def build_dataset(cfg) -> Dict[str, Callable[[], Dataset]]:
    ...
    return {
        'train': partial(HAM10000, data_dir=cfg.DATA.TRAIN_DIR, transform=train_transform),
        'val': partial(HAM10000, data_dir=cfg.DATA.VAL_DIR, transform=val_transform),
        'test': partial(HAM10000, data_dir=cfg.DATA.TEST_DIR, transform=test_transform)
    }
