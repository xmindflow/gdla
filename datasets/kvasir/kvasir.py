...
class Kvasir(Dataset):
    ...
    def _load(self, data_dir: str) -> Tuple[List[np.ndarray], List[np.ndarray]]:
        ...
        for sub_id in sub_ids:
            img_path = os.path.join(data_dir, sub_id, f"{sub_id}.png")
            msk_path = os.path.join(data_dir, sub_id, f"{sub_id}_mask.png")
...
def build_dataset(cfg) -> Dict[str, Callable[[], Dataset]]:
    ...
    return {
        'train': partial(Kvasir, data_dir=cfg.DATA.TRAIN_DIR, transform=train_transform),
        'val': partial(Kvasir, data_dir=cfg.DATA.VAL_DIR, transform=val_transform),
        'test': partial(Kvasir, data_dir=cfg.DATA.TEST_DIR, transform=test_transform)
    }
